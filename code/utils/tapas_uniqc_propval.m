function [merged unused] = tapas_uniqc_propval(propvals, defaults, varargin)

% Create a structure combining property-value pairs with default values.
%
% [MERGED UNUSED] = TAPAS_UNIQC_PROPVAL(PROPVALS, DEFAULTS, ...)
%
% Given a cell array or structure of property-value pairs
% (i.e. from VARARGIN or a structure of parameters), TAPAS_UNIQC_PROPVAL will
...
% ALTERNATIVE USAGE:
% 
% [ ARGS ] = TAPAS_UNIQC_PROPVAL(PROPVALS, DEFAULTS, ...)
%
% In this case, tapas_uniqc_propval will assume that no user specified
...
%   defaults.Y = 10;
%
%   args = tapas_uniqc_propvals(varargin, defaults)
%
%   data = data * Y / X;
% 
% >> myfunc(data)
...
%    TAPAS_UNIQC_PROPVAL will give a warning that 'foo' has no default value,
...
if ~isempty(varargin) 
  pvargs = tapas_uniqc_propval(varargin, pvdef);
end

% NOTE: Backwards compatibility with previous version of tapas_uniqc_propval
...
for i = 1:numel(properties)
  ...
  if ~ischar(properties{i}) && ~isstring(properties{1})
