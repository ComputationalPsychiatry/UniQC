function tapas_uniqc_new_script(varargin)
% Creates a new script (e.g. tests/examples) including header.
% tapas_uniqc_new_script(funname) opens the editor and pastes the content
% of a user-defined template into the file funname.m.
% Note that the prefix tapas_uniqc_ is automatically added to the filename. 
% 
%   Example
%       tapas_uniqc_new_script myscript
%           OR 
%       tapas_uniqc_new_script myscript author
%
...
% Modified: 2014-04-15 (Saskia Klein and Lars Kasper, IBT Zurich)	
	...
	switch nargin
		case 0
			edit
			warning('new_script without argument is the same as edit')
			return;
		case 1
			fname=varargin{:};
